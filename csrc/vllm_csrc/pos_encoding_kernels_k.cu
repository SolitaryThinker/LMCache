...
__global__ void rotary_embedding_kernel(
  ...
  apply_rotary_embedding<scalar_t, IS_NEOX>(
      key, cache_ptr, head_size, num_kv_heads, rot_dim,
...
void rotary_embedding_k(
    ...
    bool is_neox) {
  int64_t num_tokens = key.numel() / key.size(-1);
  ...
  VLLM_DISPATCH_FLOATING_TYPES(key.scalar_type(), "rotary_embedding", [&] {
    if (is_neox) {
      lmc::rotary_embedding_kernel<scalar_t, true><<<grid, block, 0, stream>>>(
          positions.data_ptr<int64_t>(),
          key.data_ptr<scalar_t>(), cos_sin_cache.data_ptr<scalar_t>(), rot_dim,
          key_stride, num_kv_heads, head_size);
    } else {
      lmc::rotary_embedding_kernel<scalar_t, false>
          <<<grid, block, 0, stream>>>(
              positions.data_ptr<int64_t>(),
...
void batched_rotary_embedding_k(
  ...
  VLLM_DISPATCH_FLOATING_TYPES(key.scalar_type(), "rotary_embedding", [&] {
    if (is_neox) {
      lmc::batched_rotary_embedding_kernel<scalar_t, true>
          <<<grid, block, 0, stream>>>(
              positions.data_ptr<int64_t>(),
              key.data_ptr<scalar_t>(), cos_sin_cache.data_ptr<scalar_t>(),
      ...
      lmc::batched_rotary_embedding_kernel<scalar_t, false>
          <<<grid, block, 0, stream>>>(
              positions.data_ptr<int64_t>(),
              key.data_ptr<scalar_t>(), cos_sin_cache.data_ptr<scalar_t>(),
              cos_sin_cache_offsets.data_ptr<int64_t>(), rot_dim,
